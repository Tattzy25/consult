...
function mcpCall(merchantDomain: string, tool: string, args: any) {
  return fetch(`https://${merchantDomain}/api/ucp/mcp`, {
    method: "POST",
    headers: {
  ...
  }).then((r) => r.json().then((j) => j.result?.structuredContent || j.result));
}
