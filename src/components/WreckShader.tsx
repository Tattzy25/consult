...
export const WreckShader: React.FC<WreckShaderProps> = ({ audioLevel, visualMode }) => {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      float spec = pow(
        clamp(
          dot(reflect(-viewDir, normal), vec3(0.0, 1.0, 0.0)),
          0.0001,
          1.0
        ),
        32.0
    ...
    const animate = (time: number) => {
      if (materialRef.current) {
        ...
        materialRef.current.uniforms.uAudioLevel.value +=
          (audioLevelRef.current - materialRef.current.uniforms.uAudioLevel.value) * 0.2;
        ...
        materialRef.current.uniforms.uListening.value +=
          (targetListening - materialRef.current.uniforms.uListening.value) * 0.1;

        materialRef.current.uniforms.uSpeaking.value +=
          (targetSpeaking - materialRef.current.uniforms.uSpeaking.value) * 0.1;
      }
